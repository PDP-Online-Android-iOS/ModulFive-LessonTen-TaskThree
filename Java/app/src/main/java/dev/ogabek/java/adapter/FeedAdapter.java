...
public class FeedAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final int TYPE_ADS_POST_VIEW = 3;
    ...
    public int getItemViewType(int position) {
        ...
        } else if (feeds.get(position).getPost().isAds()) {
            return TYPE_ADS_POST_VIEW;
        } else {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        } else if (viewType == TYPE_ADS_POST_VIEW) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_feed_post_ads, parent, false);
            return new AdsPostViewHolder(view);
        } else {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof AdsPostViewHolder) {
            Post post = feed.getPost();

            ((AdsPostViewHolder) holder).iv_profile.setImageResource(post.getProfile());
            ((AdsPostViewHolder) holder).iv_photo.setImageResource(post.getPhoto());
            ((AdsPostViewHolder) holder).tv_fullName.setText(post.getFullName());
    ...
    private static class AdsPostViewHolder extends RecyclerView.ViewHolder {
        ShapeableImageView iv_profile;
        TextView tv_fullName;
        ImageView iv_photo;
